...
class Stats:
    ...
    def __init__(self, processes, clock, num_cpus, num_ios, input, num_timeslice='None', simType='None'):
        ...
        self.simType = 'First Come, First Served'
        self.input = input
...
class Simulator:
    ...
    def simLoop(self, processes, num_cpus, num_ios, sleepTime): 
        ...
        # 6. check if any PCBs' in IO have current IO burst value == 0,  
            if self.IOQueue:
                ...
                for pcb in self.IOQueue: # jsut to print the still running processes in IO
            ...
            if self.waitQueue and (not self.IOQueue or len(self.IOQueue) < self.num_ios): # are processes in waitqueue & is IO not full
                ...
                for pcb in next_processes:
                    pcb.changeState('IO')
                    print(f'PCB {pcb.pid} added to IO')
    ...
    def headTable(self, input, num_cpus, num_ios) -> Table:
        ...
        headTable.add_row(f'[bold blue]Algorithm Type: [/][bold cyan] First Come, First Served w/o Priority[/]')
...
def fcfs(datafile, num_cpus, num_ios, sleepTime):
    sim = Simulator(datafile, num_cpus, num_ios, 0,  sleepTime)
