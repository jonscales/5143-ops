...
class Stats:
    ...
    """ 
    def __init__(self, processes, clock, num_cpus, num_ios, input, num_timeslice='None', simType='None'):
        """
        Stats initialization
        """
        self.processes = processes
        self.simType = "Round Robin"
        self.input = input
        self.clock = clock
...
class Simulator:
    ...
    def simLoop(self, processes, num_cpus): 
        ...
        #2. move anything in new to ready
            if self.newQueue:
                ...
                for pcb in self.readyQueue:
                    pcb.changeState('Ready')
                    print(f'PID {pcb.pid} moved to Ready')
                self.newQueue.clear()
                ...
                for pcb in self.CPUQueue:                                      # is slice time expired
                    if pcb.cpubursts: 
                        if pcb.cpubursts[0] !=0:
                            ...
                            if pcb.cpubursts[0]==0 and len(pcb.cpubursts) <= 1:                        # was this last CPU bursts for process?
                                pcb.changeState('Finished')
                                print(f'PCB {pcb.pid} DONE')
                                self.finishedQueue.append(pcb)                 # move process to finished queue
                                pcb.cpubursts.pop(0)                           # remove finished burst from cpu bursts list
        ...
        # 7. increment clock
            ...
            with Live(self.headTable(self.datfile, self.num_cpus, self.num_ios, self.timeSlice)) as live:
                live.update(self.headTable(self.datfile, self.num_cpus, self.num_ios, self.timeSlice))
    ...
    def headTable(self, input, num_cpus, num_ios, ts) -> Table:
        cpus=str(num_cpus)
        ios=str(num_ios)
        input=str(input)
        ts = str(ts)
        ...
        headTable.add_row(f'[bold blue]Algorithm Type: [/][bold cyan] Round Robin[/]')
        headTable.add_row(f'[bold deep_pink4]Time Slice: [/][bold dark_violet] {ts}[/]')
    ...
    def generateTable(self,clock) -> Table:
        ...
        table = Table(show_header=True, width=(terminal_width*.9)) # uses the add_column information to create column headings. 
...
def roundrobin(datafile, num_cpus, num_ios, ts, sleepTime):
    ...
    stats=Stats(sim.getProcesses(),sim.clock, sim.num_cpus, sim.num_ios, sim.datfile, sim.timeSlice)

if __name__=='__main__':
    roundrobin('small.dat', 2, 2, 3, .1)
